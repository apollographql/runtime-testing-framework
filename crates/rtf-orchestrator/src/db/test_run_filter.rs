...
mod tests {
    ...
    async fn runs_matching_filters_by_name() -> Result<()> {
        ...
        TestRun::init_unknown_initiator(&name, None, "alpha", c).await?;
        TestRun::init_unknown_initiator(&unique("not-this-one"), None, "alpha", c).await?;
    ...
    async fn runs_matching_filters_by_initiated_by() -> Result<()> {
        ...
        let bob = unique("bob");
        TestRun::init("a", None, Some(&alice), "alpha", c).await?;
        TestRun::init("b", None, Some(&bob), "alpha", c).await?;
    ...
    async fn runs_matching_treats_underscore_in_initiated_by_literally() -> Result<()> {
        ...
        TestRun::init("a", None, Some(&user_name), "alpha", c).await?;
        TestRun::init("b", None, Some(&user1name), "alpha", c).await?;
    ...
    async fn runs_matching_filters_by_time_range() -> Result<()> {
        ...
        let old = TestRun::init("old", None, Some(&initiated_by), "alpha", c).await?;
        let recent = TestRun::init("recent", None, Some(&initiated_by), "alpha", c).await?;
    ...
    async fn runs_matching_respects_pagination() -> Result<()> {
        ...
        for name in ["a", "b", "c"] {
            TestRun::init(name, None, Some(&initiated_by), "alpha", c).await?;
        }
    ...
    async fn runs_matching_orders_newest_first() -> Result<()> {
        ...
        let first = TestRun::init("first", None, Some(&initiated_by), "alpha", c).await?;
        TestRun::init("second", None, Some(&initiated_by), "alpha", c).await?;
    ...
    async fn runs_matching_filters_by_known_test_plan_uuid() -> Result<()> {
        ...
        let known =
            KnownTestPlan::register(&unique("plan"), None, "org", "repo", &unique("path"), c)
                .await?;
        let linked = TestRun::init_unknown_initiator(&unique("linked"), None, "alpha", c).await?;
        TestRun::init_unknown_initiator(&unique("unlinked"), None, "alpha", c).await?;
