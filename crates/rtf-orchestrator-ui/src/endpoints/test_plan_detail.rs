use crate::{
    ...
    view::{KnownTestPlanRowView, RunListView},
};
use axum::{
    Form,
...
/// submission, to re-populate the form and show the failure inline.
...
/// `GET /ui/test-plan/{uuid}` — the plan's metadata (with a GitHub link), a form to trigger a new
/// run of it, and a paginated table of its recent runs. The plan and its runs are fetched
/// concurrently, since the runs fetch only needs `uuid` from the path, not anything from the plan
/// lookup.
pub async fn handler<C: Client>(
    ...
    let (plan_result, runs_result) = tokio::join!(
...
mod tests {
    ...
    fn test_plan_detail_body_renders_the_plan_and_its_runs() {
        ...
        assert!(
            ...
            "plan should link to its file on GitHub, got: {body}"
        );
        assert!(
            body.contains("<form"),
