...
fn build_output_zip(ctx: &impl CliContext, shared_dir: &Path, zip_path: &Path) -> CliResult<()> {
    ...
    let mut buf = Vec::new();
    {
        ...
        for file_path in files {
            // strip_prefix cannot fail here: list_files_under guarantees files are under output_dir
            // which is under shared_dir. Bridged through FilesystemError until a dedicated
