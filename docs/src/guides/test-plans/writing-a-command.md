...
remove the values and matrix added in the final step of the "Writing a test plan" guide. Your
...
`scenario.sh` file in the `output/providers` directory matches what was defined in the `scenario.sh`
file we just created. Copying files in this way is intentional behavior for the `relative_path` file
provider, other providers have files generated by rtf directly, this is one of the few that
references files created before rtf runs. It guarantees the path where the file will be located so
