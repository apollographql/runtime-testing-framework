...
| `rtf.io/otel`           | `true` | Injects REP collector endpoints into the container as environment variables. The following variables are set automatically: `REP_OTEL_COLLECTOR_GRPC` (gRPC endpoint, port 4317) and `REP_OTEL_COLLECTOR_HTTP` (HTTP/protobuf endpoint, port 4318). Use these in your service's config instead of the standard `OTEL_*` variables. |
