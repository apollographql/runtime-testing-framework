...
executable file that can be used as one of the environment setup, environment teardown or scenario
commands. Within config files they are wrapped in a [CommandSpec][2] which allows the user to
provide command line arguments.

Each command section supports specifying environment variables and file providers that will be made
available when the command executes.
